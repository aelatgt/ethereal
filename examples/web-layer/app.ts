...
const todoVue = (window as any).todoVue = vue.createApp(TodoMVC).mount('') as ComponentPublicInstance<typeof TodoMVC>

// handle routing (use Vue's reactive properties to update the DOM)
function onHashChange() {
  const visibility = window.location.hash.replace(/#\/?/, '') as keyof typeof filters 
...
function updateRay(x:number, y:number) {
