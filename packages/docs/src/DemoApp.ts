...
import { EtherealSystem } from '@etherealjs/core'
...
export class DemoApp extends AppBase {
    ...
    constructor() {
        super({
            onUpdate: (event) => {
                BehaviorManager.update(this.scene, this.camera, event.deltaTime)
            },
            onEnterXR: (event) => {
        ...
        BehaviorManager.addBehavior(this.scene, () => {
        ...
        BehaviorManager.addBehavior(this.cameraDolly, ()=> {
            ...
            BehaviorManager.ensureUpdate(this.target)
        ...
        this.belowRoomTarget.position.y = 0
        this.belowRoomTarget.rotateX(Math.PI/2*0.99)
        this.belowRoomTarget.add(new THREE.AxesHelper(2))
        ...
        this.roomTarget.add(new THREE.AxesHelper)
