...
test('adapter w/ target bounds, no parent, default orientation, w/ empty inner size', t => {
    ...
    t.deepEqual(state.relativePosition, new Vector3(1,2,0))
    t.deepEqual(state.relativeScale, new Vector3(4,5,6))
...
test('adapter w/ target bounds, parent w/ geometry, orientation, and custom inner size', t => {
    ...
    t.deepEqual(parentState.relativePosition,parentBox.position)
    t.deepEqual(parentState.relativeScale, new Vector3(1,2,2))
    ...
    function setupContent() {
        ...
        t.deepEqual(state.relativePosition, new Vector3(0,0,0))
        t.deepEqual(state.relativeOrientation, adapter.orientation.target)
        t.deepEqual(state.relativeScale, new Vector3(0.25,0.25,0.5))
