...
import { memoizeTracked, DirtyableTag } from '@glimmer/validator'
...
export function memo(target:any, key:string|symbol, desc:PropertyDescriptor): PropertyDescriptor {
  ...
  return {
    get() {
      ...
      if (memoizedFn === undefined) {
        memoizedFn = memoizeTracked(fn.bind(this))
