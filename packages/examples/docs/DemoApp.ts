...
import { system, adapt, state, easing, transition, SphericalCoordinate, Q_IDENTITY, V_010, V_111 } from 'ethereal'
...
export class DemoApp extends AppBase {
        ...
        adapt(this.camera, adapter => {
            ...
            adapter.onUpdate = () => {
                if (!this.xrPresenting) {
                    const dollyState = state(this.dolly)
    ...
    loadRoom() {
        ...
        this.gltfLoader.load(`${this.publicUrl}static/models/stylized_room/scene-lowpoly.gltf`, (gltf) => {
            ...
            adapt(roomLines, (adapter) => {
                adapter.onUpdate = ()=>{
                    lineMaterial.resolution = this.resolution
                }
            })
            
            const adapter = ethereal.system.getAdapter(app.room)!
            adapter.metrics.invalidateIntrinsicBounds()
        })
    }
