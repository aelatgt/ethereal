...
// redeclare popmotion types here so that the consumer doesn't have to 
...
//     export const reversed: EasingModifier;
//     export const mirrored: EasingModifier;
//     export const createReversedEasing: EasingModifier;
//     export const createMirroredEasing: EasingModifier;
...
//     export const bounceIn: (p: number) => number;
//     export const bounceInOut: (p: number) => number;
//     export function cubicBezier(mX1: number, mY1: number, mX2: number, mY2: number): (aX: number) => number;
// }

// const easing = (_easing as any) as typeof Easings

export {easing}

function defined(a:any,b:any,c:any) {
    return  typeof a !== 'undefined' ? a : 
            typeof b !== 'undefined' ? b :
            typeof c !== 'undefined' ? c : undefined
}
...
export class Transitionable<T extends ValueType = ValueType> {
    ...
    private _addTargetInfluence(start:WidenLiteral<T>, target:TransitionTarget<WidenLiteral<T>>) {
        const alpha = target.duration > 0 ? target.easing( Math.max(Math.min(target.elapsed, target.duration) / target.duration, 1) ) : 1
        ...
        if (typeof target.value === 'number') {
            this.current += MathUtils.lerp(0, target.value-(start as number), alpha) as any
            return
        } 
